...
def main():
    ...
    datasets = ['Music_emotion','Music_style']
    # =======================参数设置==================================
    preProcessMethod = ['minMax']  # minMax, standard, mM_std, std_mM
    param_list_alpha = [1,10,100]
    param_beta = [0.1,1,10]
    k = [3,4,5,6]

    temp = list(itertools.product(datasets, noise_ratio_list, preProcessMethod, param_list_alpha, param_beta, k))  # 笛卡尔积 进行参数排列组合 每个元组（数据集名称,参数1,参数2，...）
    temp.reverse()
...
def SingleProcess(parameter_dist):
    ...
    while True:
        ...
        share_lock.release()
        print("==== process {}, dataset:{}, noise:{},{}, alpha:{},beta:{},k:{} time:{}".format(processing_index, dataset_name, noise_ratio, preProcessMethod,param_alpha, param_beta,param_k,time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
        ...
        try:
            ...
            if preProcessMethod == 'minMax':
                ...
                print("数据未经过预处理")
            features_rank, run_time = FS.FeatureSelection(X=X,Y=Y,param_alpha=param_alpha,param_beta=param_beta,param_k=param_k,dataset_name=dataset_name,noise_ratio=noise_ratio)
            ...
            note = open("{}/{}_{}_{}.txt".format(file_path, param_alpha,param_beta,param_k), mode='w')
            ...
            with open(record_filepath, 'a', newline='') as f:
                ...
                writer = csv.writer(f)
                writer.writerow([dataset_name, X.shape[0], X.shape[1], Y.shape[1],noise_ratio, preProcessMethod,str(param_alpha),str(param_beta),str(param_k), 'Exception:' + str(e), '',time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), socket.gethostname()])
        ...
        with open(record_filepath, 'a', newline='') as f:
            fcntl.flock(f.fileno(), fcntl.LOCK_EX)  # 排他锁
            writer = csv.writer(f)
            writer.writerow([dataset_name, X.shape[0], X.shape[1], Y.shape[1],noise_ratio, preProcessMethod,str(param_alpha), str(param_beta),str(param_k), len(features_rank), run_time, time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), socket.gethostname()])
            fcntl.flock(f, fcntl.LOCK_UN)
    share_lock.release()
